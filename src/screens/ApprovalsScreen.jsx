...
const ApprovalsScreen = ({ onBack, onNavigate, onLogout, language, onLanguageChange }) => {
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50 lg:ml-0">
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 p-4 lg:p-6">
          <div className="max-w-7xl mx-auto">
            {/* Mobile Card Layout for Pending Phases */}
            <div className="lg:hidden space-y-4">
              {pendingPhases.map((phase) => (
                <div key={phase.id} className="bg-white rounded-xl shadow-md border border-gray-200 p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex-1">
                      ...
                      <p 
                        ...
                        style={{ fontFamily: 'Tajawal, ui-sans-serif, system-ui' }}
                      >
                        {isRTL ? `المرحلة: ${phase.phaseNameAr}` : `Phase: ${phase.phaseName}`}
                      </p>
                    </div>
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">{isRTL ? 'تم الإرسال بواسطة:' : 'Submitted by:'}</span>
                      <span className="text-gray-700 font-medium">{isRTL ? phase.submittedByAr : phase.submittedBy}</span>
                    </div>
                    <div className="flex items-center justify-between text-sm">
                      <span className="text-gray-500">{isRTL ? 'تاريخ الإرسال:' : 'Submission date:'}</span>
                      <span className="text-gray-700">{phase.submissionDate}</span>
                    </div>
                  </div>
                  
                  <button
                    onClick={() => handleReview(phase)}
                    className="w-full px-4 py-2 bg-gradient-to-r from-[#67AF31] to-[#8BC34A] text-white font-semibold rounded-lg hover:shadow-lg transition-all duration-200 text-sm"
                    style={{ fontFamily: 'Tajawal, ui-sans-serif, system-ui' }}
                  >
                    {isRTL ? 'مراجعة' : 'Review'}
                  </button>
